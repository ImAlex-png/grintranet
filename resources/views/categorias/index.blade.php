...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-8">
        ...
        <a href="{{ route('categorias.create') }}" class="bg-sky-600 hover:bg-sky-700 text-white font-bold py-2.5 px-6 rounded-xl transition duration-300 shadow-lg flex items-center gap-2">
    ...
    <div class="glass-panel rounded-2xl p-4 mb-8">
        <form action="{{ route('categorias.index') }}" method="GET" id="filter-form" class="flex flex-wrap items-center gap-3">
            ...
            <div class="relative dropdown-container">
                ...
                <div class="dropdown-content shadow-xl">
                    <div class="p-3 space-y-2">
                        @php $tipos = ['departamento', 'curso', 'tipo_documento']; @endphp
                        @foreach($tipos as $tipo)
                            <label class="flex items-center gap-3 p-2 hover:bg-blue-900/30 rounded-lg cursor-pointer transition-all">
                                <input type="checkbox" name="tipos[]" value="{{ $tipo }}" 
                                       ...
                                       {{ is_array(request('tipos')) && in_array($tipo, request('tipos')) ? 'checked' : '' }}>
                                <span class="text-xs font-medium text-gray-200">{{ ucfirst(str_replace('_', ' ', $tipo)) }}</span>
                            </label>
                        @endforeach
                    </div>
                </div>
            ...
            <!-- Limpiar -->
            @if(request()->anyFilled(['buscar', 'tipos']))
                <a href="{{ route('categorias.index') }}" class="p-2.5 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-all" title="Limpiar filtros">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
